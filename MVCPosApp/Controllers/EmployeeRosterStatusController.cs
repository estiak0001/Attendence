    ...
    public class EmployeeRosterStatusController : Controller
    {
        ...
        public JsonResult SaveEvent(EventTable e)
        {
            ...
            using (GCTL_ERP_DB_MVC_06_27Entities dc = new GCTL_ERP_DB_MVC_06_27Entities())
            {
                ...
                {
                    //Update the event
                    var v = dc.EventTables.Where(a => a.EventID == e.EventID).FirstOrDefault();
                    if (v != null)
                    {
                        v.Subject = e.Subject;
        ...
        public JsonResult DeleteEvent(int eventID)
        {
            ...
            using (GCTL_ERP_DB_MVC_06_27Entities dc = new GCTL_ERP_DB_MVC_06_27Entities())
            {
                var v = dc.EventTables.Where(a => a.EventID == eventID).FirstOrDefault();
                if (v != null)
                {
                    dc.EventTables.Remove(v);
                    dc.SaveChanges();
                    status = true;
                }
            }
